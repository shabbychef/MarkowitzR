...
do.all <- function(rets,...) {
	set.coln(rets)
	retval <- rbind(bjones.ts(rets),wrap.itheta(rets,...),wrap.mp(rets,...))
	rownames(retval) <- c("Britten Jones t-stat","via itheta_vcov","via mp_vcov")
	return(retval)
}
n.day <- 2000
n.stock <- 5
...
rets <- matrix(rnorm(n.day * n.stock,mean=5e-1),nrow=n.day)
...
# try with robust s.e.
if (require(sandwich)) {
	print(do.all(ff.ret,vcov.func=sandwich::vcovHAC))
}
@
