...
The \txtMP appears in the inverse of a certain matrix
containing first and second moments of asset returns. Using this
`weird trick', the asymptotic distribution of the 
\txtMP can be found via the central limit theorem and delta method.
This allows one to construct Wald statistics on the elements of the
Markowitz portfolio or perform shrinkage on those elements.
This derivation is more general than that of 
Britten-Jones \cite{BrittenJones1999}, as it allows the use of 
robust standard errors; can be extended to deal with hedged portfolios,
conditional hetereskedasticity and expectation; allows estimation
of the proportion of error due to mis-estimation of the covariance matrix.
Use of this computatio via the \MarkowitzR package is illustrated.
\end{abstract}%UNFOLD
...
\subsection{Code and Basic Usage}%FOLDUP
Here I present example usage of the above results in R. In each
scenario, the results of primary interest are the marginal Wald
test statistics under the null of zero weight in the \txtMP. 
The basic procedure is listed here:
...
wtrick.ws <- function(rets,...) {
	set.coln(rets)
	# 'flipping the sign on returns is idiomatic'
	asymv <- itheta_vcov(- as.matrix(rets),...)
	qidx <- 2:asymv$pp
	wald.s <- asymv$mu[qidx] / sqrt(diag(asymv$Ohat[2:asymv$pp,2:asymv$pp]))
  names(wald.s) <- colnames(rets)
	return(wald.s)
}
...
do.both <- function(rets,...) {
	set.coln(rets)
	retval <- rbind(bjones.ts(rets),wtrick.ws(rets,...))
	rownames(retval) <- c("Britten Jones t-stat","weird trick Wald stat")
	return(retval)
}
@
...
procedure. \cite{BrittenJones1999} The tests are performed on 256
observations of 7 assets, first under the null of zero mean returns,
...
set.seed(as.integer(charToRaw("04fa9bec-f342-47c0-85a6-692324a25f3f")))
...
Example code showing normality:
<<'normality',echo=TRUE>>=
...
%set.seed(as.integer(charToRaw("1289daaa-6060-4901-bc90-c5245bcf73f1")))
...
\subsection{Fama French Three Portfolios}
