...
opts_chunk$set(fig.width=5,fig.height=4,dpi=300)
...
the `Markowitz Coefficient' which linearly scales the features
into a portfolio. \cite{pav2013markowitz}

Here we generate such a population, then check the Markowitz
Coefficient
...
Here is a scatter of the fit value against the true value of the
Markowitz Coefficient:
...
marko.ws <- function(rets,ret.what=c('wald','mp'),...) {
	set.coln(rets)
...
The \txtMP is produced by the same code:

<<'cons_two',echo=TRUE,cache=FALSE>>=
...
rets <- matrix(rnorm(n.day * n.stock,mean=1e-1),nrow=n.day)
Gmat <- matrix(1,nrow=1,ncol=n.stock)
print(marko.ws(rets,ret.what='mp',Gmat=Gmat))
# and in the case where it is not.
set.seed(as.integer(charToRaw("420f1dfd-b19b-4175-83b3-b96548264bf8")))
rets <- matrix(rnorm(n.day * n.stock,mean=0),nrow=n.day)
mu.off <- t(matrix(seq(from=-0.8,to=0.8,length.out=n.stock),nrow=n.stock,ncol=n.day))
