...
covariance that may be estimated from the data. For essentially no extra
computational work, one also gets an estimate of the 
...
{\code itheta\_vcov}; a fancier version is given by 
{\code marko\_vcov}. The primary use case of the variance-covariance
...
the implementation of {\code itheta\_vcov} and the Wald test.
...
portfolio equal dollar long in each of the fake assets. The Wald statistics are
computed as follows

<<'cons_one',echo=TRUE,cache=FALSE>>=
marko.ws <- function(rets,...) {
	set.coln(rets)
	# 'flipping the sign on returns is idiomatic'
	asymv <- marko_vcov(as.matrix(rets),...)
	wald.s <- asymv$W / sqrt(diag(asymv$What))
	return(t(wald.s))
}
...
mu.off <-
t(matrix(seq(from=-0.8,to=0.8,length.out=n.stock),nrow=n.stock,ncol=n.day))
rets <- rets + mu.off
Gmat <- matrix(1,nrow=1,ncol=n.stock)
print(marko.ws(rets,Gmat=Gmat))
@
...
<<'normality',echo=TRUE,cache=FALSE>>=
...
%zero.rets <- t(matrix(rnorm(n.day * n.stock),nrow=n.day))
%mu.vals <- (1/sqrt(253)) * seq(-1,1,length.out=n.stock) 
