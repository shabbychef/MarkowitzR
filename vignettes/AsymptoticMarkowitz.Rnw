...
\lemmaref{sr_optimal_portfolio} to get the following analogue; to simplify
notation, we will need the following projection operator.

\begin{definition}[Risk Projection]
\label{definition:risk_projection}
Define the covariance-projection operator as
\begin{equation}
\proj{\pvsig}{\Mtx{A}} \defeq \wrapProj{\pvsig}{\Mtx{A}},
\end{equation}
for conformable matrix \Mtx{A}.
\end{definition}
...
\dbyd{\proj{\pvsm}{\zerJt}}{\pvsm}
&=
\dbyd{\proj{\pvsm}{\zerJt}}{\minv{\wrapParens{\qoform{\pvsm}{\zerJt}}}} 
...
Finally, we can combine the subspace and hedging constraints with the 
conditional model of
\subsecref{cond_ret_het}. This extension is trivial only in the case
...
To perform inference on the hedged Markowitz coefficient, we need to consider
the difference of two projected second moment matrices.
...
Once again, the delta inverse second moment contains the Markowitz coefficient,
...
where we define
\begin{equation*}
\broj{\pvsm}{\Mtx{A}} \defeq
\wrapParens{\AkronA{\tr{\Mtx{A}}}}
\wrapParens{\AkronA{\minv{\wrapParens{\qoform{\pvsm}{\Mtx{A}}}}}}
